    ...
    describe('Sanitization', () => {
      it('should use "sanitizeHtml" to sanitize template', () => {
        const factory = new TemplateFactory({
          sanitize: true,
          template: '<div><a href="javascript:echo alert(7)">Click me</a></div>'
        })
        const spy = spyOn(factory, '_maybeSanitize').and.callThrough()

        expect(factory.toHtml().innerHTML).not.toContain('href="javascript:echo alert(7)')
      ...
      it('should not sanitize template', () => {
        ...
        expect(factory.toHtml().innerHTML).toContain('href="javascript:echo alert(7)')
        expect(spy).toHaveBeenCalled()
      })
      ...
      it('should use "sanitizeHtml" to sanitize content', () => {
        const factory = new TemplateFactory({
          ...
          content: { '#foo': '<a href="javascript:echo alert(7)">Click me</a>' }
        })
        expect(factory.toHtml().innerHTML).not.toContain('href="javascript:echo alert(7)')
      })
